...
cv.my_alas <- function(Y,X,G){
  ...
  for (i in 1:lam_len){
    val_ind = which(las_fit$beta[-c(1,2), (lam_len - i + 1)] == 0)
    # we will only consider the models with greater than 2 IVs.
    if (any(is.na(val_ind)) | length(val_ind)<=2) {next}
    mod = lm(S[val_ind, val_ind]^(1/2) %*% by[val_ind] ~ S[val_ind, val_ind]^(1/2) %*% bx[val_ind] - 1)
    ivw_est= mod$coefficients
    w= bx[val_ind]^2/se_y[val_ind]^2
    chival <- qchisq(0.95,df=length(val_ind)-1)
    # out is a matrix with four rows. The first row contains the Cochran'Q stats, and the second row is for the 
